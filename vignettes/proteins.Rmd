...
databases created through the Ensembl Perl API contain protein annotation,
databases created using `ensDbFromAH`, `ensDbFromGff`, `ensDbFromGRanges` and
`ensDbFromGtf` don't contain such information.
...
```{r a_2_select_nmd, eval = haveProt}
...
```{r b_proteins_mcols, eval = haveProt}
mcols(prts)
```
...
```{r b_proteins_prot_doms, eval = haveProt}
...
```{r b_proteins_prot_doms_2, eval = haveProt}
...
Protein mapping functionality is provided by the `Pbase` Bioconductor package.
