...
region on chromosome Y. Note that we also have to change the
`ucscChromosomeNames` option from `Gviz`, as the chromosome names from Ensembl
are named differently from UCSC (i.e. without `chr`).
...
## To allow use of Ensembl chromosome names we have to change a default option
## in Gviz:
...
shown in Figure [88](#orgparagraph1)):
