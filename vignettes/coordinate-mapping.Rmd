...
- `transcriptToGenome`: maps coordinates within a transcript to genomic
  ...
  the database (and thus can not be mapped to the genome).
...
empty `GRanges` if the transcript identifier can not be found in the
database). The length of each `GRanges` depends on the number of exons the
region in the transcript spans.
