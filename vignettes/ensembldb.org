...
# eventually using a filter to restrict the query. Alternatively, we could also
# extract the =GRanges= representing the exons of each transcript and use the
# =getSeq= method.

To retrieve the (exonic) sequence of transcripts (i.e. without introns) we first
fetch all exons by transcript and use the =extractTranscriptSeqs= method from
the =GenomicRanges= package to extract the sequences from the genomic =FaFile=
fetched above.
...
respectively Gff file which can be downloaded from Ensembl (e.g. from
...
** TODO Implement some more =GenomicFeatures= methods [/]

+ [ ] =transcriptLenghts=: use the =lengthOf= method.
...
** TODO Include functionality from the =GenomeInfoDb= to fix chromosome naming.

+ [ ] Implement a =seqlevelsStyle<-= method for =EnsDb=. Should do something
  similar than the stuff for =Gviz=.
...
+ [ ] Implement a =seqlevelsStyle= method for =EnsDb=.
+ [ ] Implement central =mapSlToEns= =mapSlFromEns= methods (basically
  replacement for =ucscToEns= and =prefixChromName=).
+ [ ] =EnsDb= needs a new slot to store any data (type list).
...
** TODO Allow more generic GTF file names in =ensDbFromGtf=
