...
=ensembldb.seqnameNotFound= set to "ORIGINAL"), the original seqnames (i.e. the ones
...
** TODO Implement a =SymbolFilter= and support a =symbol= column

** TODO What about using pipe and /formula-like/ filters?

** TODO Fix the =select= method such that it always returns the values in the same order than the keys were
...
** TODO *Always* return the attribute of the filter!
