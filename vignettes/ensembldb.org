...
Using the Ensembl version provided by the =EnsDb=, the correct genomic sequence can however be retrieved easily from the =AnnotationHub=.

First we load the =AnnotationHub= data and list all resources available for the specific Ensembl version and species. Next we load the =dna.toplevel.fa= fasta file and retrieve the sequences for all genes defined in the =EnsDb= package. These sequences represent the sequence between the chromosomal start and end coordinates of the gene.
...
** TODO Implement a function to load the appropriate DNA sequence from AnnotationHub

+ [ ] Implement a method to retrieve the Ensembl version.
Some code snippet:
