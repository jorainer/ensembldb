...
\date{Modified: 19 June, 2015. Compiled: \today}
...
\section{Interactive annotation lookup using the \texttt{shiny} web app}
\label{sec:orgheadline5}

In addition to the \texttt{genes}, \texttt{transcripts} and \texttt{exons} methods it is possibly to
search interactively for gene/transcript/exon annotations using the internal,
\texttt{shiny} based, web application. The application can be started with the
\texttt{runEnsDbApp()} function.
