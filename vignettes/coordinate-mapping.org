...
To verify that the nucleotide at the position is indeed a /G/ as stated in the
definition of the variant (/c.1643delG/) we extract below the nucleotide at the
identified genomic position. We thus load the package providing the genome
sequence for GRCh38 on which Ensembl release 86 is based.
