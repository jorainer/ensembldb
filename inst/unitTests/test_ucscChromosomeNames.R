...
test_SeqNameFilter_seqnames <- function(){
    ...
    checkEquals(value(snf), "chrX")
    checkEquals(value(snfEns), c("X", "Y"))
    seqlevelsStyle(edb) <- "UCSC"
    checkEquals(ensembldb:::ensDbQuery(snf, edb), "gene.seq_name = 'X'")
    checkException(ensembldb:::ensDbQuery(snfEns, edb))
    checkException(ensembldb:::ensDbQuery(snfNo, edb))
    checkException(ensembldb:::ensDbQuery(snfSomeNo, edb))

    ## Setting the options to "ORIGINAL"
    options(ensembldb.seqnameNotFound="ORIGINAL")
    checkEquals(ensembldb:::ensDbQuery(snf, edb), "gene.seq_name = 'X'")
    suppressWarnings(
        checkEquals(ensembldb:::ensDbQuery(snfEns, edb),
                    "gene.seq_name in ('X','Y')")
    )
    suppressWarnings(
        checkEquals(ensembldb:::ensDbQuery(snfNo, edb),
                    "gene.seq_name in ('bla','blu')")
    )
    suppressWarnings(
        checkEquals(ensembldb:::ensDbQuery(snfSomeNo, edb),
                    "gene.seq_name in ('bla','X')")
    )
    ##
    snf <- SeqNameFilter(c("chrX", "Y"))
    suppressWarnings(
        checkEquals(ensembldb:::ensDbQuery(snf, edb),
                    "gene.seq_name in ('X','Y')")
    )
    options(ensembldb.seqnameNotFound=orig)
}
